...
function downloadScript() {
  ...
  const psLines = [
    ...
    "try {",
    "    $usbAudio = Get-CimInstance Win32_PnPEntity -ErrorAction Stop | Where-Object {",
    "        $_.DeviceID -match '^USB' -and $_.Status -eq 'OK' -and $_.Name -match 'Audio|Headset|Headphone|Microphone|Mic|Speaker'",
    "    }",
    "    if ($usbAudio) {",
    "        $usbHeadset = ($usbAudio | Select-Object -First 1).Name",
    "    }",
    "    $usbMic = $usbAudio | Where-Object { $_.Name -match 'Microphone|Mic|Headset' } | Select-Object -First 1",
